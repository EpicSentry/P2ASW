...
void CHLClient::LevelInitPreEntity(char const* pMapName)
{
	...
	// Time for else if hell!! fun
	int currentAct = 0; // Initialize the variable to store the current act
	...
	if (currentAct != 0)
	{
		char line[256]; // Increase buffer size to accommodate longer lines if needed
		FILE* autoexecFile;
		fopen_s(&autoexecFile, "..\\cfg\\autoexec.cfg", "r+");
		if (autoexecFile)
		{
			std::string cfgContent;
			bool portal2CurrentActFound = false; // Flag to check if the line exists
			while (fgets(line, sizeof(line), autoexecFile))
			{
				if (strstr(line, "portal2_current_act") == line)
				{
					Msg("Modifying current act");
					// Replace the existing portal2_current_act value
					sprintf_s(line, sizeof(line), "portal2_current_act %d\n", currentAct);
					portal2CurrentActFound = true;
				}
				cfgContent += line;
			}
			fclose(autoexecFile);

			// If the portal2_current_act line doesn't exist, add it to the end of the content
			if (!portal2CurrentActFound)
			{
				Msg("Current act missing or invalid, repairing");
				sprintf_s(line, sizeof(line), "portal2_current_act %d\n", currentAct);
				cfgContent += line;
			}

			fopen_s(&autoexecFile, "..\\cfg\\autoexec.cfg", "w");
			if (autoexecFile)
			{
				fputs(cfgContent.c_str(), autoexecFile);
				fclose(autoexecFile);
			}
		}
	}
