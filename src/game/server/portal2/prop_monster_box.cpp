...
const char *s_pAnimateThinkContext = "AnimateThinkContext";
...
void CPropMonsterBox::Spawn( void )
{
	...
	SetBodygroup(2, (m_nBodyGroups >> 1) & 1);
	SetBodygroup(3, (m_nBodyGroups >> 2) & 1);
	SetBodygroup(4, (m_nBodyGroups >> 3) & 1);
	...
	else
	{
		if (VPhysicsGetObject())
		{
			...
			if (vecUp.z >= 0.1)
			{
				...
				trace_t tr;
				UTIL_TraceLine( GetAbsOrigin(), vecAbsEnd, 0x200400Bu, this, COLLISION_GROUP_NONE, &tr);
				if (tr.fraction != 1.0)
				{
					Vector angImpulse = RandomAngularImpulse( -0.5, 0.5 );

					float flRandom = RandomFloat(-0.2, 0.2);

					vecAbsEnd.z = RandomFloat(0.69999999, 1.0);
					...
					vecVelocity.x = ( (vecForward.x + flRandom) * flVel ) * m_flPushStrength;
...
void CPropMonsterBox::AnimateThink( void )
{
	...
	else
	{
			...
			+ (vecVelocity.z * vecVelocity.z)) <= (m_flBoxSwitchSpeed * m_flBoxSwitchSpeed))
		{
			...
			{
				Vector v23 = CollisionProp()->OBBMins();
				Vector v24 = CollisionProp()->OBBMaxs();

				trace_t tr;

				UTIL_TraceHull( GetAbsOrigin(), GetAbsOrigin(), v23, v24, 0x200400Bu, this, 0, &tr);
				if (tr.startsolid || tr.allsolid)
				{
					Vector vecImpulse;
					Vector vecUp;
					GetVectors( 0, 0, &vecUp );
					Vector angImpulse = RandomAngularImpulse( -0.5, 0.5);
		...
		if (m_bSequenceFinished)
		{
			...
			else
			{
				...
				if (vecUp.z >= 0.60000002)
				{
					...
					trace_t tr;
					UTIL_TraceLine( GetAbsOrigin(), vecImpulse, 0x200400Bu, this, 0, &tr );
					if (tr.fraction == 1.0)
					{
...
{
	// Wtf? Linux has this function but windows doesn't, use BaseClass code for now.
#if 0
	if (pActivator)
	{
