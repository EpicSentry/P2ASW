						   ...
						   IShaderDynamicAPI *pShaderAPI, IShaderShadow* pShaderShadow, PortalRefractVars_t &info, VertexCompressionType_t vertexCompression )
{
	...
	SHADOW_STATE
	{
		...
		// Pixel Shader
		if( g_pHardwareConfig->SupportsPixelShaders_2_b() ) // Always send OpenGL / Posix down this path
		{
	...
	DYNAMIC_STATE
	{
		...
		if ( nTinted )
		{
			pShaderAPI->SetPixelShaderConstant( 7, IS_PARAM_DEFINED( info.m_nPortalColorGradientDark ) ? params[info.m_nPortalColorGradientDark]->GetVecValue() : kDefaultPortalColorGradientDark, 1 );
			pShaderAPI->SetPixelShaderConstant( 8, IS_PARAM_DEFINED( info.m_nPortalColorGradientLight ) ? params[info.m_nPortalColorGradientLight]->GetVecValue() : kDefaultPortalColorGradientLight, 1 );
		}
		...
		pShaderAPI->SetPixelShaderConstant( 9, vViewportMad, 1 );
		...
		pShaderAPI->SetVertexShaderConstant( VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, vPackedConst4, 1 );
	}
	pShader->Draw();
}
